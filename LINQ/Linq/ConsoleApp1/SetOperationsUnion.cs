...
namespace SetOperationsUnionns
{
    class SetOperationsUnion
    {
        static void Main1(string[] args)
        {
            ...
            var querySyntax = (from d in datasource1
                               select d).Union(datasource2).ToList();
            ...
            //Union on Name
            var methodSyntax2 = student.Select(s => s.Name).Union(student2.Select(s2 => s2.Name)).ToList();
            foreach (var item in methodSyntax2)
                Console.WriteLine(item);

            Console.WriteLine("--------------------------------------------------");
            //Union with anonumas method
            var methodSyntac3 = student.Select(s => new { s.Id, s.Name }).Union(student2.Select(s2 => new { s2.Id, s2.Name })).ToList();
